...
const BookAppointment = ({date}) => {
  return (
    <section className="container">
      <h3 className="text-center text-info">Available Appointment On {date.toDateString()}</h3>
      <div className="row py-5">
        {
          bookingData.map((booking, idx)=> <BookingCard key={idx} booking={booking} date={date} /> )
        }
      </div>
    </section>
  )
